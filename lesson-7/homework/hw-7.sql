...
SELECT MAX (SALARY) as highers_salary
from Employees
...
select sum (SaleAmount) as sales_id_7
from Sales
group by ProductID
Having ProductID = 7; 
...
select min(price) as min_price_category,
	Category
	from Products
	group by Category;

select max(price) as max_price_category,
	Category
	from Products
	group by Category;
...
select sum(SaleAmount) as sales_per_customer,
	CustomerID
	from Sales
	group by CustomerID;

--10. Write a query to filter departments having more than 5 employees 
--from the Employees table.(DeptID is enough, if you don't have DeptName).
select * from Employees
...
select ProductID, sum (saleamount) as total_sales_per_productID
from Sales
group by ProductID;

select ProductID, avg (saleamount) as total_sales_per_productID
from Sales
group by ProductID;
...
select count(employeeID) as staff_per_dept_HR,
	DepartmentName
	from Employees
	group by DepartmentName
...
select avg(salary) as avg_salary_per_dept,
	DepartmentName
	from Employees
	group by DepartmentName;
...
select departmentname,
	avg(salary) as avg_salarydept,
	count (employeeID) as empl_dept 
	from Employees
	group by departmentname; 
...
select 
	customerID,
	count (customerID) as customers_orders
	from sales 
	group by CustomerID
	having sum(SaleID) > 3; 
	...
	select 
	customerID,
	count (distinct saleID) as customers_orders
	from sales 
	group by CustomerID
	having sum(SaleID) > 3; 
...
-- 20. Write a query that shows the average price for each product category, and then filter 
-- categories with an average price greater than 150.
select * from Products
select 
	category, 
	avg(price) as aver_price_per_category 
	from Products
	group by category;
...
-- 21. Write a query to calculate the total sales for each Customer, then filter the results 
-- to include only Customers with total sales over 1500.
...
SELECT
    custid,
    SUM(case when freight > 50 THEN freight ELSE 0 END) AS freights_above_50,
...
-- 25. Write a query that calculates the total sales and counts unique products sold 
-- in each month of each year, and then filter the months with at least 2 products sold.(Orders)
select * from sales 
